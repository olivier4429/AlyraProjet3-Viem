...
const WORKFLOW_ACTIONS: Record<number, { label: string; fn: WorkflowFunction }> = {
...
export default function WorkflowManager() {
  const { isOwner, isOwnerLoading, isConnected, workflowStatus, workflowLabel, refetchAll } =
    useApp();
  ...
  const action =
    workflowStatus !== undefined ? WORKFLOW_ACTIONS[Number(workflowStatus)] : undefined;
      ...
      <CardContent>
        <div className="space-y-2">
          <div>
            ...
            <p className="text-lg text-blue-600 font-semibold mt-1">{workflowLabel}</p>
          </div>
          {workflowStatus !== undefined && (
            <p className="text-sm text-gray-500">Phase {workflowStatus}/5</p>
          )}
        </div>
      </CardContent>
      <CardFooter className="flex-col gap-2">
        {!isLoading && action && (
          <Button variant="default" className="w-full" onClick={advanceWorkflow}>
            → {action.label}
          </Button>
        )}
        {!isLoading && !action && (
          <p className="text-green-600 font-semibold">✓ Workflow terminé</p>
        )}
        {isLoading && <p className="text-blue-600">Transaction en cours...</p>}
        {isError && <p className="text-red-600 text-sm">Erreur : {error?.message}</p>}
        {isConfirmed && (
          <p className="text-green-600 text-sm">✓ Transition effectuée avec succès !</p>
        )}
      </CardFooter>
    </Card>
  );
